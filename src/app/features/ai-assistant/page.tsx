...
export default function AIAssistantPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-white pt-24 text-slate-900 selection:bg-blue-100 selection:text-blue-900 font-inter">
      <InteractiveGrid />
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[400px] lg:h-[540px] bg-[radial-gradient(circle_at_top,rgba(5,160,236,0.12),transparent_38%),radial-gradient(circle_at_top_right,rgba(9,53,140,0.08),transparent_25%)]" />
      ...
      <section className="relative py-12 md:py-20 4xl:py-30">
        <Container>
          <div className="grid items-center gap-8 lg:gap-16 lg:grid-cols-[0.92fr_1.08fr]">
            <div>
              <motion.div {...fadeUp()} className="mb-6 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-4 py-2 text-xs font-black uppercase tracking-[0.22em] text-[#09358c] shadow-sm">
              ...
              <motion.div {...fadeUp(0.24)} className="mt-10 flex flex-wrap items-center gap-4">
                <Link href="/enterprise" className="group inline-flex items-center gap-3 rounded-full bg-gradient-to-r from-[#09358c] to-[#05a0ec] px-8 py-4 text-base font-black text-white shadow-[0_22px_60px_-18px_rgba(9,53,140,0.45)] transition-all hover:scale-[1.05] hover:shadow-[0_25px_70px_-15px_rgba(9,53,140,0.55)]">
                  <Play size={16} fill="currentColor" className="transition-transform group-hover:rotate-12" /> Book a Demo
                </Link>
                <div className="flex flex-col">
                  <span className="text-sm font-normal text-slate-500">Retail intelligence via</span>
            ...
            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.3 }} className="rounded-[2rem] lg:rounded-[3rem] border border-slate-800 bg-slate-950 p-6 shadow-[0_50px_130px_-60px_rgba(15,23,42,0.95)] relative overflow-hidden group">
              <div className="absolute inset-0 bg-gradient-to-b from-[#05a0ec]/10 to-transparent pointer-events-none" />
              ...
              <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" className="mt-8 space-y-5 relative z-10">
                ...
                <motion.div variants={bubbleVariant("right")} className="ml-auto max-w-[80%] rounded-[2rem] rounded-tr-md bg-gradient-to-r from-[#09358c] to-[#05a0ec] p-5 text-base font-black leading-relaxed text-white shadow-xl shadow-blue-500/20">
                ...
                <motion.div variants={bubbleVariant("left")} className="max-w-[90%] rounded-[2rem] rounded-tl-md bg-white/8 p-6 border border-white/5 shadow-inner">
                  ...
                  <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: 1 }} className="mt-6 rounded-2xl border border-white/10 bg-slate-900/80 p-5 group/card hover:bg-slate-800 transition-colors">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#05a0ec]">Contextual Insight Card</p>
                    ...
                    <div className="mt-3 h-2.5 rounded-full bg-white/10 overflow-hidden">
                      <motion.div initial={{ width: 0 }} whileInView={{ width: "72%" }} transition={{ duration: 1.5, delay: 1.2, ease: "circOut" }} className="h-full rounded-full bg-gradient-to-r from-emerald-500 via-[#05a0ec] to-[#09358c]" />
                    </div>
                  </motion.div>
                </motion.div>
              </motion.div>
                ...
                <div className="flex items-center justify-between gap-6 px-2">
                  ...
                  <div className="flex items-end gap-1.5 h-6">
                    {[12, 22, 34, 18, 12, 26, 14].map((height, index) => (
                      <motion.span key={index} animate={{ height: [height, height * 1.5, height] }} transition={{ duration: 0.8 + index * 0.1, repeat: Infinity, ease: "easeInOut" }} className="w-1.5 rounded-full bg-[#05a0ec] opacity-60 group-hover/mic:opacity-100 transition-opacity" />
                    ))}
                  </div>
                </div>
              </motion.div>
        ...
        <Container>
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="grid gap-6 lg:grid-cols-3">
            {[["Business problem", "Critical answers stay trapped behind dashboards, filters, and people who know how to navigate them.", "text-rose-500"], ["What Tapito detects", "Tapito detects intent, business context, and follow-up meaning inside voice or chat questions.", "text-[#09358c]"], ["What action it enables", "Anyone can get live business intelligence instantly, even while moving through the store floor.", "text-emerald-600"]].map(([title, text, tone], index) => (
              <motion.div key={title} {...fadeUp(index * 0.1)} {...cardHover} className={`rounded-[2.5rem] border bg-white p-10 group transition-all ${index === 1 ? "border-blue-200 shadow-[0_40px_90px_-50px_rgba(5,160,236,0.2)]" : "border-slate-200 hover:border-blue-200"}`}>
      ...
      <section className="py-24 relative overflow-hidden font-inter">
        <div className="absolute top-0 right-0 w-2/3 h-full bg-[radial-gradient(circle_at_top_right,rgba(5,160,236,0.03),transparent_70%)] pointer-events-none" />
        <Container>
              ...
              {[["01", "Ask naturally", "Speak or type the same way you would ask an analyst or lead."], ["02", "Interpret context", "Tapito understands retail language, time windows, and follow-up chains."], ["03", "Respond with move", "Answers include the metric, the shift, and the suggested next move."]].map(([step, title, text], index) => (
                <motion.div key={step} {...fadeUp(index * 0.15)} {...cardHover} className="rounded-[2rem] border border-slate-200 bg-white p-8 group hover:bg-slate-50 transition-colors h-full">
                  <div className="flex items-start gap-6">
                    <motion.div whileHover={{ scale: 1.1, rotate: 10 }} className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-slate-950 text-base font-black text-white group-hover:bg-[#05a0ec] transition-colors shadow-lg">{step}</motion.div>
                    <div>
            ...
            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.16 }} className="rounded-[3.5rem] border border-blue-200 bg-gradient-to-br from-white via-blue-50/30 to-indigo-50/50 p-10 lg:p-14 relative group">
              ...
              <div className="grid gap-6 md:grid-cols-2 relative z-10">
                <motion.div {...fadeUp(0.6)} {...cardHover} className="rounded-[2rem] border border-slate-800 bg-slate-950 p-8 shadow-xl group/card">
                   ...
                   <div className="mt-8 px-4 py-5 rounded-2xl bg-white/5 border border-white/5 relative overflow-hidden text-white/90 font-black group-hover/card:border-[#05a0ec]/30 transition-colors">
                     "What changed in Central Hub?"
                     <motion.div animate={{ x: ["-100%", "100%"] }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }} className="absolute bottom-0 left-0 w-1/3 h-[2px] bg-gradient-to-r from-transparent via-[#05a0ec] to-transparent" />
                   </div>
                </motion.div>
                <motion.div {...fadeUp(0.7)} {...cardHover} className="rounded-[2rem] border border-blue-100 bg-white p-8 shadow-sm group/card">
      ...
      <section className="py-16 4xl:py-24 relative overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-[radial-gradient(circle,rgba(5,160,236,0.02),transparent_70%)] pointer-events-none" />
        <Container>
        ...
        <Container>
          ...
          <div className="grid overflow-hidden rounded-[3.5rem] border border-slate-200 shadow-[0_50px_140px_-60px_rgba(15,23,42,0.4)] md:grid-cols-2 relative h-full">
            ...
            <motion.div initial={{ x: 100, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1 }} className="bg-slate-950 p-12 lg:p-20 text-white relative h-full group">
               <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(5,160,236,0.1),transparent_70%)] pointer-events-none" />
